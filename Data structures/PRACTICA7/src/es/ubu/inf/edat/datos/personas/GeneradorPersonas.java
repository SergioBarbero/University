...
public class GeneradorPersonas {
	...
	public static Persona[] soloTrabajadores(){
		...
		// Se añade alguno mezclado
		for (int m = 0; m<4; m++){
			int j = (int) Math.floor(Math.random()*apellidosTr.length);
			int k = (int) Math.floor(Math.random()*apellidosTr.length);
			int l = (int) Math.floor(Math.random()*apellidosTr.length);
		
			Trabajador t = new Trabajador(apellidosTr[j], nombreTr[k], nifTr[l], generoTr[j], edadTr[k], expedienteTr[l]);
			t.setServicio(generaServicio());
			t.setMesesAntiguedad(generaAleatorio(120));
			lp.add(t);
		}
	...
	public static String generaEstudio(){
